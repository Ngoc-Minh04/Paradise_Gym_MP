...
CREATE TABLE IF NOT EXISTS chi_nhanh (
    ...
    ma_chi_nhanh    TEXT    NOT NULL UNIQUE, -- CN001, CN002
    ten_chi_nhanh   TEXT    NOT NULL,
    dia_chi         TEXT,
    so_dien_thoai   TEXT,
    trang_thai      TEXT    NOT NULL DEFAULT 'hoat_dong' CHECK (trang_thai IN ('hoat_dong','tam_dung')),
...
INSERT OR IGNORE INTO vai_tro (ma_vai_tro, ten_hien_thi, mo_ta, quyen_json) VALUES
...
CREATE TABLE IF NOT EXISTS ho_so (
    ...
    chi_nhanh_id    INTEGER REFERENCES chi_nhanh(id) DEFAULT 1,
...
CREATE TABLE IF NOT EXISTS doanh_thu (
    ...
    ghi_chu         TEXT,
    ngay_cap_nhat   DATETIME NOT NULL DEFAULT (datetime('now','localtime')),
    UNIQUE(chi_nhanh_id, ngay)
);

-- ============================================================
-- 11B. CẤU HÌNH HỆ THỐNG
-- ============================================================
CREATE TABLE IF NOT EXISTS cau_hinh_he_thong (
    id              INTEGER PRIMARY KEY AUTOINCREMENT,
    chi_nhanh_id    INTEGER REFERENCES chi_nhanh(id) DEFAULT 1,
    ma_cau_hinh     TEXT    NOT NULL, -- 'may_cham_cong_ip', 'may_cham_cong_port'
    gia_tri         TEXT    NOT NULL,
    mo_ta           TEXT,
    UNIQUE(chi_nhanh_id, ma_cau_hinh)
);
...
CREATE TRIGGER IF NOT EXISTS trg_doanh_thu_goi_tap
    ...
    INSERT OR IGNORE INTO doanh_thu (chi_nhanh_id, ngay, tong_tien, tong_don, tien_goi_tap, tien_goi_pt)
    VALUES (
        (SELECT chi_nhanh_id FROM ho_so WHERE id = NEW.ho_so_id),
        date('now','localtime'), NEW.gia_thuc_te, 1, NEW.gia_thuc_te, 0
    )
    ON CONFLICT(chi_nhanh_id, ngay) DO UPDATE SET
...
CREATE TRIGGER IF NOT EXISTS trg_doanh_thu_goi_pt
    ...
    INSERT OR IGNORE INTO doanh_thu (chi_nhanh_id, ngay, tong_tien, tong_don, tien_goi_tap, tien_goi_pt)
    VALUES (
        (SELECT chi_nhanh_id FROM ho_so WHERE id = NEW.hoi_vien_id),
        date('now','localtime'), NEW.gia_thuc_te, 1, 0, NEW.gia_thuc_te
    )
    ON CONFLICT(chi_nhanh_id, ngay) DO UPDATE SET
        tong_tien   = tong_tien + NEW.gia_thuc_te,
        tong_don    = tong_don + 1,
...
INSERT OR IGNORE INTO tai_khoan (ten_dang_nhap, mat_khau_hash, vai_tro_id) VALUES
...
INSERT OR IGNORE INTO ho_so (tai_khoan_id, ma_ho_so, loai_ho_so, ho_ten, gioi_tinh, ngay_sinh, so_dien_thoai, email, avatar_url, nguoi_tao_id) VALUES
...
-- Gói tập
INSERT OR IGNORE INTO goi_tap (ten_goi, so_thang, so_ngay_them, gia, mo_ta, nguoi_tao_id) VALUES
...
-- Gói PT
INSERT OR IGNORE INTO goi_pt (ten_goi, loai_goi, so_buoi, so_thang, gia, nguoi_tao_id) VALUES
...
INSERT OR IGNORE INTO cham_cong (ho_so_id, ngay, gio_vao, gio_ra, nguon, raw_data) VALUES
...
-- Audit log mẫu
INSERT OR IGNORE INTO audit_log (tai_khoan_id, ten_dang_nhap, vai_tro, hanh_dong, doi_tuong, doi_tuong_id, gia_tri_cu, gia_tri_moi, ip_address, ghi_chu) VALUES
