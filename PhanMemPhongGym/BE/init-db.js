...
try {
  ...
  // Tách phần schema ra khỏi phần SEED DATA một cách linh hoạt
  const separatorRegex = /--\s*=+[\r\n]+--\s*DỮ LIỆU MẪU \(SEED\)[\r\n]+--\s*=+[\r\n]*/;
  const schemaParts = sqlContent.split(separatorRegex);
  ...
  // Chi nhánh
  db.exec(`
    INSERT OR IGNORE INTO chi_nhanh (id, ma_chi_nhanh, ten_chi_nhanh, dia_chi, so_dien_thoai) VALUES
    (1, 'CN001', 'Paradise Gym - Quận 1', '123 Lê Lợi, Q.1, TP.HCM', '0901234567'),
    (2, 'CN002', 'Paradise Gym - Quận 7', '456 Nguyễn Thị Thập, Q.7, TP.HCM', '0907654321');
  `);
  console.log('✅ Đã tạo 2 chi nhánh mẫu');
  ...
  // Hồ sơ
  db.exec(`
    INSERT OR IGNORE INTO ho_so (id, tai_khoan_id, ma_ho_so, loai_ho_so, chi_nhanh_id, ho_ten, gioi_tinh, ngay_sinh, so_dien_thoai, email, avatar_url, nguoi_tao_id) VALUES
    (1, 1, 'AD001', 'nhan_vien', 1, 'Nguyễn Văn Admin',  'nam', '1990-01-15', '0901000001', 'admin@paradise.gym',   'https://i.pravatar.cc/150?img=11', 1),
    (2, 2, 'LT001', 'le_tan',    1, 'Trần Thị Lễ Tân',   'nu',  '1998-05-20', '0901000002', 'letan01@paradise.gym', 'https://i.pravatar.cc/150?img=5',  1),
    (3, 3, 'PT001', 'pt',        1, 'Lê Văn Tuấn',        'nam', '1995-03-10', '0901000003', 'pt01@paradise.gym',    'https://i.pravatar.cc/150?img=67', 1),
    (4, 4, 'PT002', 'pt',        2, 'Phạm Thị Lan',       'nu',  '1996-07-22', '0901000004', 'pt02@paradise.gym',    'https://i.pravatar.cc/150?img=56', 1),
    (5, 5, 'HV001', 'hoi_vien',  1, 'Võ Văn Minh',        'nam', '2000-09-05', '0901000005', 'hv01@gmail.com',       'https://i.pravatar.cc/150?img=12', 2),
    (6, 6, 'HV002', 'hoi_vien',  2, 'Ngô Thị Hoa',        'nu',  '1999-12-18', '0901000006', 'hv02@gmail.com',       'https://i.pravatar.cc/150?img=47', 2),
    (7, 7, 'HV003', 'hoi_vien',  1, 'Đặng Văn Khoa',      'nam', '2001-04-30', '0901000007', 'hv03@gmail.com',       NULL,                               2);
  `);
  console.log('✅ Đã tạo hồ sơ mẫu');

  // Gói tập & Gói PT
  db.exec(`
    INSERT OR IGNORE INTO goi_tap (id, chi_nhanh_id, ten_goi, so_thang, so_ngay_them, gia, mo_ta, nguoi_tao_id) VALUES
    (1, 1, 'Gói 1 tháng',          1,  0,  500000, 'Tập không giới hạn trong 1 tháng', 1),
    (2, 1, 'Gói 3 tháng',          3,  0, 1350000, 'Tiết kiệm 10% so với gói 1 tháng', 1),
    (3, 2, 'Gói 6 tháng',          6,  0, 2400000, 'Tiết kiệm 20% so với gói 1 tháng', 1),
    (4, 1, 'Gói 12 tháng',        12,  0, 4200000, 'Tiết kiệm 30% — Gói năm tốt nhất', 1),
    (5, 1, 'Gói 1 tháng + 5 ngày', 1,  5,  550000, 'Tháng + thêm 5 ngày khuyến mãi',  1);

    INSERT OR IGNORE INTO goi_pt (id, chi_nhanh_id, ten_goi, loai_goi, so_buoi, so_thang, gia, nguoi_tao_id) VALUES
    (1, 1, 'PT 10 buổi',      'theo_buoi',  10, NULL, 1500000, 1),
    (2, 1, 'PT 20 buổi',      'theo_buoi',  20, NULL, 2800000, 1),
    (3, 2, 'PT 1 tháng',      'theo_thang', NULL, 1,  2000000, 1),
    (4, 1, 'PT 3 tháng',      'theo_thang', NULL, 3,  5500000, 1),
    (5, 1, 'PT Nhóm 10 buổi', 'theo_buoi',  10, NULL,  800000, 1);
  `);
  ...
  db.exec(`
    INSERT OR IGNORE INTO doanh_thu (chi_nhanh_id, ngay, tong_tien, tong_don, tien_goi_tap, tien_goi_pt) VALUES
    (1, date('now','-6 days'), 3500000, 4, 2000000, 1500000),
    (1, date('now','-5 days'), 2800000, 3, 1800000, 1000000),
    (2, date('now','-4 days'), 4200000, 5, 2700000, 1500000),
    (1, date('now','-3 days'), 1500000, 2,  500000, 1000000),
    (1, date('now','-2 days'), 5500000, 6, 3500000, 2000000),
    (2, date('now','-1 days'), 3200000, 4, 2200000, 1000000);
  `);
